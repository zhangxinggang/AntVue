  ...
  methods: {
    ...
    createPage(route) {
      return {
        ...
        unclose: route.meta && route.meta.closable === false
      }
    },
    /**
     * 设置页面缓存的key
    ...
    setCachedKey(route) {
      ...
      page.unclose = route.meta && route.meta.closable === false
      if (!page._init_) {
     ...
     */
    loadCachedTabs() {
      const cachedTabsStr = sessionStorage.getItem('admin.tabs')
      if (cachedTabsStr) {
        try {
