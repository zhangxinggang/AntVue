...
const {theme,storageOptions,constant} = require('../config')
...
function getLocalSetting(loadTheme) {
  let localSetting = {}
  const localSettingStr = localStorage.getItem(storageOptions.namespace+constant.sysSettingName)
  if(localSettingStr!=null && localSettingStr!='null' && localSettingStr!=undefined && localSettingStr!='undefined'){
    localSetting = JSON.parse(localSettingStr).value
  }
  if (loadTheme) {
